...
- **Tool discovery and schemas**: The memory-disabled default surface exposes 34 active tools, each with an object-root MCP `outputSchema`. `sdl.info` provides runtime and capability discovery; optional memory tools remain outside that default surface.
