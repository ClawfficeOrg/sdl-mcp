...
- **Stable tool contracts**: Default `sdl.repo.status` responses separate stable repository and graph state from opt-in `includeTelemetry` operational data. `sdl.context` enforces its whole-response budget with a 512-token request minimum, exposes continuation retrieval for truncated context, and resolves precise explicit implementation intent through implementation-oriented handlers. Byte-stability checks use inline context with `refsMode: "off"`; response-artifact handles and session refs remain deliberately session-scoped.
...
- **SDL tool QA follow-up**: Exact and focused context keeps implementation evidence on the first page, scoped explain requests add hot paths, and insufficient answer-first evidence returns an honest partial status. Explicit slice entries remain authoritative roots while graph traversal expands relationships, and disconnected slices return deterministic recovery guidance. Invalid response paths retain sorted valid keys and same-handle retry calls. TypeScript SCIP metadata now preserves interface kinds and declaration-wide multiline variable ranges across cards and code/edit windows.
