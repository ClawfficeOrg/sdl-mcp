...
export const AGENT_OUTPUT_CASES = [
  ...
  compactCase({
    ...
    canonicalResultFactory: () => ({
      formattedSummary: "1 projected call",
    }),
    expectedCompactKeys: ["formattedSummary"],
    requiredActionabilityKeys: ["formattedSummary"],
    executionMode: "read-only",
  }),
  compactCase({
    action: "file.read",
  ...
  compactCase({
    ...
    expectedCompactKeys: ["feedback", "aggregatedStats", "hasMore"],
