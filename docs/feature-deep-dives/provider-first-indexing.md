...
The same verification boundary covers parser provenance. `FileParserState` records the engine, engine contract, adapter key, and language for every indexed file, while `RepoParserState` binds exact file-state coverage to the graph version and revision. Final publication writes complete repository coverage and graph verification in one transaction; neither state advances alone.
