...
Keep the production and test runtime defaults at 30 seconds. Preserve `SYSTEMROOT` in the scrubbed Windows runtime environment, matching the repository's existing safe-spawn environment and allowing nested native commands to initialize reliably.

## Verification

Add a focused benchmark assertion that a Windows artifact records and applies the 1,600-millisecond ceiling while the default threshold remains 1,000 milliseconds. Add a Windows unit regression for `SYSTEMROOT`, then run the benchmark unit test, runtime executor unit test, targeted runtime integration test, typecheck, and lint before pushing.
