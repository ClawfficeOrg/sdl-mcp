export const SDL_MCP_SERVER_INSTRUCTIONS = [
  ...
  "12. Do not wait for semantic freshness or graph verification unless the current task explicitly requires latest-revision graph proof. On a parser-provenance failure, reindex only if AST/provenance-dependent behavior is required; otherwise use an SDL file-based fallback or report the limitation. Subagents may report the need, but only the root agent may request approval or initiate a refresh.",
